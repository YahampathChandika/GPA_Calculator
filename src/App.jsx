...
function App() {
  const [courses, setCourses] = useState([]);
  const [allYearsGpa, setAllYearsGpa] = useState(null);
  const [threeYearsGpa, setThreeYearsGpa] = useState(null);
  const [gpaBreakdown, setGpaBreakdown] = useState([]);
  ...
  const calculateWeightedGPA = () => {
    const calculateGpaByMethod = (method) => {
      const selectedWeights = yearWeights[method];
      let weightedGPA = 0;
      let breakdown = [];

      for (const year in selectedWeights) {
        let totalPoints = 0;
        let totalCredits = 0;
        ...
        const yearGPA = totalCredits ? totalPoints / totalCredits : 0;
        const weightedYearGPA = yearGPA * selectedWeights[year];
        weightedGPA += weightedYearGPA;
        breakdown.push({
          year,
          yearGPA: yearGPA.toFixed(2),
          weightedYearGPA: weightedYearGPA.toFixed(2),
          weight: selectedWeights[year],
        });
      }

      return { weightedGPA: weightedGPA.toFixed(2), breakdown };
    };

    const allYearsResult = calculateGpaByMethod("allYears");
    const threeYearsResult = calculateGpaByMethod("threeYears");

    setAllYearsGpa(allYearsResult.weightedGPA);
    setThreeYearsGpa(threeYearsResult.weightedGPA);
    setGpaBreakdown(allYearsResult.breakdown); // Or adjust if needed
  };
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {Object.entries(gradePoints).map(([grade, point]) => (
                <tr key={grade}>
                  <td className="border-t p-2 w-1/2 text-center">{grade}</td>
                  <td className="border-t p-2 w-1/2 text-center">
                    {point.toFixed(1)}
                  </td>
                </tr>
              ))}
        ...
        <div className="mt-6 w-full max-w-md bg-white p-4 rounded-lg shadow-md">
          ...
          {gpaBreakdown.map((item) => (
            ...
            >
              ...
              <p>Year GPA: {item.yearGPA}</p>
              <p>Weighted GPA for Year: {item.weightedYearGPA}</p>
            </div>
          ))}
          <p className="mt-4 text-sm text-gray-600">
            * GPA calculated based on both methods:
            <br />
            - All Years: 10% for 1st year, 20% for 2nd year, 30% for 3rd year,
            and 40% for 4th year.
            <br />- Last 3 Years: 20% for 2nd year, 30% for 3rd year, and 50%
            for 4th year.
          </p>
        </div>
      )}
